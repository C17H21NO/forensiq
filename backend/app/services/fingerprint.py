...
def build_fingerprint(
    ...
    # Limpieza de boilerplate antes de construir las representaciones.
    clean_text = strip_boilerplate(text)

    text_vector = build_text_vector(clean_text)

    # SBERT se conserva únicamente para reproducibilidad de experimentos
    # históricos/camera-ready. El flujo productivo OE3 utilizará
    # include_semantic=False y no cargará el modelo.
    if include_semantic:
        from app.services.semantic_encoder import build_semantic_embedding
    ...
    pii_features = normalize_feature_vector(
        build_pii_features(pii_entities)
    )
...
def compare_fingerprints(
    ...
    query_semantic = query_fp.get("semantic_text_vector", [])
    candidate_semantic = candidate_fp.get("semantic_text_vector", [])

    if query_semantic and candidate_semantic:
        from app.services.semantic_encoder import cosine_similarity_dense
...
# =============================================================================
# Flujo productivo OE3 / TP2
# =============================================================================
#
# La aplicación final separa claramente:
#
# - Texto: señal utilizada para ordenar el ranking Top-k.
# - Layout: evidencia estructural auxiliar.
# - PII: contexto de identidad, fuera del score.
#
# SBERT se conserva para experimentos históricos, pero no participa
# en el flujo productivo validado con usuarios.
# =============================================================================

PRODUCTION_RANKING_MODEL = "lexical_text_similarity_v1"
...
def build_production_fingerprint(
    ...
    """
    Construye la huella utilizada por la aplicación OE3.

    No ejecuta SBERT.
    Layout y PII se conservan como evidencia auxiliar/contextual.
    """
    ...
    fingerprint["ranking_model"] = PRODUCTION_RANKING_MODEL

    fingerprint["evidence_roles"] = {
        "text": "ranking",
        "layout": "auxiliary",
        "pii": "context_only",
    }
...
def compare_production_fingerprints(
    query_fp: Dict[str, Any],
    candidate_fp: Dict[str, Any],
) -> Dict[str, Any]:
    """
    Comparación utilizada en el flujo productivo OE3.

    ranking_score:
        Señal utilizada para ordenar el Top-k.

    layout_score:
        Evidencia estructural auxiliar. No modifica ranking_score.

    pii_context_score:
        Coincidencia contextual de identidad. No modifica ranking_score.

    Los campos final_score y pii_score permanecen temporalmente como aliases
    para no romper el frontend TP1 mientras hacemos la migración.
    """

    text_score = cosine_similarity(
        query_fp.get("text_vector", {}),
        candidate_fp.get("text_vector", {}),
    )

    layout_score = cosine_similarity(
        query_fp.get("layout_features", {}),
        candidate_fp.get("layout_features", {}),
    )

    pii_context_score = cosine_similarity(
        query_fp.get("pii_features", {}),
        candidate_fp.get("pii_features", {}),
    )

    # Única señal utilizada para ordenar candidatos en la versión OE3.
