...
from app.services.fingerprint import (
    ...
    compare_production_fingerprints,
)
...
# =============================================================================
# Matcher productivo OE3 / TP2
# =============================================================================
#
# Este flujo se utiliza exclusivamente en la aplicación que será validada
# con usuarios.
#
# Diferencia respecto al matcher experimental:
#
# - El ranking se ordena únicamente mediante ranking_score.
# - ranking_score corresponde actualmente a similitud textual léxica.
# - layout_score se conserva como evidencia estructural auxiliar.
# - pii_context_score se conserva como información contextual.
# - Layout y PII NO modifican el orden Top-k.
# - No se utilizan pesos de fusión.
# =============================================================================


def match_against_references_production(
    query_fingerprint: Dict[str, Any],
    top_k: int = 5,
) -> List[Dict[str, Any]]:
    """
    Compara una huella sospechosa contra el índice de referencias utilizando
    exclusivamente el contrato productivo de OE3.

    El resultado se ordena por ranking_score descendente.

    Los empates se resuelven mediante el mismo mecanismo determinista basado
    en hash utilizado por el matcher experimental, evitando depender del
    orden de inserción de los documentos.
    """

    results: List[Dict[str, Any]] = []

    for ref in REFERENCE_INDEX:
        scores = compare_production_fingerprints(
            query_fingerprint,
            ref["fingerprint"],
        )
    ...
    results.sort(
        key=lambda item: (
            -item["scores"]["ranking_score"],
            _tiebreak_key(item["document_id"]),
        )
    )
...
def match_all_references_production(
    query_fingerprint: Dict[str, Any],
) -> List[Dict[str, Any]]:
    """
    Variante productiva que devuelve todos los documentos del índice.

    Está pensada principalmente para pruebas internas y validación técnica.
    """

    return match_against_references_production(
        query_fingerprint=query_fingerprint,
        top_k=len(REFERENCE_INDEX),
    )
